...
class OrderProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'product', 'product_id', 'quantity', 'price', 'status', 'status_display')
...
class OrderProductStatusUpdateSerializer(serializers.Serializer):

    """Serializer for updating order product status"""

    status = serializers.ChoiceField(

        choices=['Ожидание', 'Обрабатывается', 'Отправлен'],

        required=True,

        help_text='Новый статус товара в заказе: Ожидание, Обрабатывается, Отправлен'
